...
from apps.dummy2.task.dummy2taskstate import Dummy2TaskDefaults, Dummy2TaskOptions
...
class Dummy2Task(CoreTask):
    ...
    the corresponding password. It will do so by comparing each charset's permutation
    hash with the original one. The task will be split into subtasks, each working 
    on it's assigned permutation's range (linear).
    To ensure subtask computation authenticity each subtask will be given not one but multiple 
    hashes. These hashes will correspond to passwords randomly picked from problem space. This allows 
    to inject real hash into the subtask without the provider knowing which one it really is, stopping him from
    returning early (e.g. when finding all the artificial hashes). This way forces provider to return full set of passwords
    assigned to his subtask. 

    Attributes:
        artificial_passwds  helper Dict{password: Tuple(bucket_index, sha256_hexdigest)}, bucket_index helps to find all artificial passwords corresponding to the given subtask 
        hashes_merged shuffled list of hashes containing both artificial hashes and the original problem hash 
        password_length original problem password length
        password_hash original problem password hash 
        real_password original problem password (to be determined, set by verifier class)

    Input data file:
    password_length sha256_hex_digest

    Corner cases:
    Not finding a password results in a silent finish. That could happen if password uses different character set.
    ...
    def __init__(self,
        ...
        shared_data = open(self.task_definition.shared_data_files[0], 'r').read()
        ...
        self.artificial_passwds = self.get_artificial_passwds(
                            functools.partial(self.get_charset_permutations, self.password_length),
                            total_tasks)
        self.hashes_merged = [item[1] for item in self.artificial_passwds.values()] + [passwd_hash]
    ...
    def _extra_data(self, perf_index=0.0, start_task = 1) -> ComputeTaskDef:
        ...
        subtask_string = "{}\n{}\n{}\n{}\n".format(self.password_length, start_task - 1, self.total_tasks, len(self.hashes_merged))
    ...
    def get_artificial_passwds(self, permutations_generator, steps):
    ...
    def accept_results(self, subtask_id, result_files):
        ...
        #One possible scenario would be to write an error message to output file
...
class Dummy2TaskBuilder(CoreTaskBuilder):
    TASK_CLASS = Dummy2Task

    @classmethod
    def build_dictionary(cls, definition: Dummy2TaskDefinition):
    ...
    @classmethod
    def build_full_definition(cls, task_type: Dummy2TaskTypeInfo, dictionary):

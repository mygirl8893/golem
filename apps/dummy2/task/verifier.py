...
from apps.dummy2.resources.code_dir import computing
...
class Dummy2TaskVerifier(CoreVerifier):
    ...
    def _verify_result(self, results: Dict[str, Any]):
        ...
        original_passwds = set(parent_task.get_passwds_for_bucket(subtask_offset))
        ...
        if not original_passwds == result_passwds:
            #Mismatch subset must be a single element set containing the password of interest
            #otherwise result is malicious

...
class Dummy2TaskBenchmark(CoreBenchmark):
    def __init__(self):
        ...
        self.dummy2_task_path = join(get_golem_path(),
                                    "apps", "dummy2", "test_data")

        td = self._task_definition = Dummy2TaskDefinition(Dummy2TaskDefaults())
        td.shared_data_files = [join(self.dummy2_task_path, x) for x in
        ...
        self.verifier = Dummy2TaskVerifier(lambda **kwargs: None,
                                          verification_data)
